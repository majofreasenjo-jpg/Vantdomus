...
export function AuthScreen({ navigation }: any) {
  // SECURITY: never pre-populate real credentials. The previous version of
  // this screen shipped `test@demo.com` / `demo123` hardcoded which would
  // leak into release builds. Leave the inputs blank.
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      // JWT lands in SecureStore (Keychain/Keystore) on native; falls back
      // to AsyncStorage only on web. The household id is not sensitive and
