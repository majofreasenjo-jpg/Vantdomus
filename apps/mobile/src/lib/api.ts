...
async function getToken() {
  // JWT lives in SecureStore (Keychain/Keystore) on native; falls back to
  // AsyncStorage only on web. See lib/secureStore.ts for details.
...
// Auth
// SECURITY: credentials travel in the JSON body, NOT in the URL query string.
// The previous version put email+password in the URL, which leaks through
// proxy/CDN access logs, browser history, crash reporters, and server logs.
...
export const getCeoState = () => apiFetch(`/ceo/dashboard`);
