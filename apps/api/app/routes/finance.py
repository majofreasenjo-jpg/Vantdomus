...
def add_expense(household_id: str, amount: float, currency: str="USD", category: str="general",
    ...
    db.execute("""
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
    """, (eid, household_id, organization_id, float(amount), currency, category, merchant, expense_at, notes, person_id, created))
    write_audit_log(
        db,
        action="create",
        ...
        resource_id=eid,
        metadata={"amount": amount, "currency": currency, "category": category, "merchant": merchant},
    )
    db.commit()
    return {"id": eid}
