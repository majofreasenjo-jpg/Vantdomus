...
def set_plan(household_id: str, person_id: str, med_name: str, reminder_times: str, verification_mode: str="none",
    ...
    db.execute("INSERT INTO events (id,household_id,organization_id,domain,event_type,occurred_at,summary,payload,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
               (ev_id, household_id, organization_id, "health", "adherence_plan_set", now(), f"Plan adherencia set: {med_name}", payload, now()))
...
def checkin(household_id: str, person_id: str, med_name: str, status: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    db.execute("INSERT INTO events (id,household_id,organization_id,domain,event_type,occurred_at,summary,payload,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
               (ev_id, household_id, organization_id, "health", "medication_checkin", ts, f"Check-in {med_name}: {status}", payload, ts))
    db.execute("INSERT OR IGNORE INTO event_actors (event_id, person_id, role) VALUES (?,?,?)", (ev_id, person_id, "patient"))
    ...
    if new >= 2:
        ...
        db.execute("""
          INSERT OR IGNORE INTO alerts (id,household_id,organization_id,severity,event_id,title,message,status,dedupe_key,created_at)
          VALUES (?,?,?,?,?,?,?,?,?,?)
        """, (str(uuid.uuid4()), household_id, organization_id, "high", ev_id, f"Riesgo de no adherencia: {med_name}",
