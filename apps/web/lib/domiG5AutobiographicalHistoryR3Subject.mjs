...
export const DOMI_G5_R3_SUBJECT_VERSION = "DOMI_G5_R3_SUBJECT_V0_1";
export const DOMI_G5_R3_ENTRY_IDS = Object.freeze([
  "G5-E-0001-REAL",
  "G5-E-0002-REAL",
  G5_THIRD_REAL_OWNER_ENTRY_ID,
]);
export const DOMI_G5_R3_ACTION_SPACE = Object.freeze(
  Array.from({ length: 64 }, (_, index) => `R3_ACTION_${index.toString(16).toUpperCase().padStart(2, "0")}`),
);
export const DOMI_G5_R3_SALTS = Object.freeze({
...
function validateHistory(value) {
  if (!Array.isArray(value) || value.length === 0) {
    return Object.freeze({ ok: false, disposition: "HOLD_HISTORY_UNAVAILABLE", byId: new Map() });
  }
  const byId = new Map();
  for (const memory of value) {
    if (!isCanonicalGovernedEntry(memory) || byId.has(memory.entryId)) {
      return Object.freeze({ ok: false, disposition: "HOLD_UNGOVERNED_HISTORY", byId: new Map() });
    }
    byId.set(memory.entryId, memory);
  }
  if (byId.size !== DOMI_G5_R3_ENTRY_IDS.length) {
    return Object.freeze({ ok: false, disposition: "HOLD_HISTORY_INCOMPLETE", byId });
  }
  for (const id of DOMI_G5_R3_ENTRY_IDS) {
    if (!byId.has(id)) return Object.freeze({ ok: false, disposition: "HOLD_HISTORY_INCOMPLETE", byId });
  }
  return Object.freeze({ ok: true, disposition: null, byId });
}

function chronologicalTrajectory(byId) {
  const ordered = DOMI_G5_R3_ENTRY_IDS.map((id) => byId.get(id)).sort((a, b) => {
    const delta = Date.parse(a.observedAt) - Date.parse(b.observedAt);
    if (delta !== 0) return delta;
    return a.entryId.localeCompare(b.entryId);
  });
  if (ordered.map((entry) => entry.entryId).join("|") !== DOMI_G5_R3_ENTRY_IDS.join("|")) {
    throw new Error("G5_R3_CANONICAL_TEMPORAL_ORDER_INVALID");
  }
  return ordered;
}

function trajectoryDigestFromFingerprints(fingerprints) {
  let state = sha256Text(`${DOMI_G5_R3_SALTS.trajectory}|ROOT|${fingerprints.length}`);
  fingerprints.forEach((fingerprint, index) => {
    state = sha256Text(`${DOMI_G5_R3_SALTS.trajectory}|${index}|${state}|${fingerprint}`);
  });
  return state;
}

function actionFromTrajectory({ selector, nonce, trajectoryDigest }) {
  const permutation = [...DOMI_G5_R3_ACTION_SPACE].sort((a, b) => {
    const rankA = sha256Text(`${DOMI_G5_R3_SALTS.permutation}|${trajectoryDigest}|${a}`);
    const rankB = sha256Text(`${DOMI_G5_R3_SALTS.permutation}|${trajectoryDigest}|${b}`);
    return rankA === rankB ? a.localeCompare(b) : rankA.localeCompare(rankB);
  });
  const offset = Number.parseInt(
    sha256Text(`${DOMI_G5_R3_SALTS.offset}|${nonce}|${trajectoryDigest}`).slice(0, 8),
    16,
  ) % DOMI_G5_R3_ACTION_SPACE.length;
  return permutation[(selector * 17 + offset) % DOMI_G5_R3_ACTION_SPACE.length];
}
...
export function getG5R3SubjectFreezeDescriptor() {
  return Object.freeze({
    version: DOMI_G5_R3_SUBJECT_VERSION,
    entryIds: DOMI_G5_R3_ENTRY_IDS,
    ...
    actionSpace: DOMI_G5_R3_ACTION_SPACE,
    salts: DOMI_G5_R3_SALTS,
    providerReadPolicy: "EXACTLY_ONCE_PER_SUBJECT_CALL",
    canonicalOrder: "OBSERVED_AT_ASC_THEN_ENTRY_ID",
    trajectoryConstruction: "ITERATED_SHA256_ALL_THREE_ENTRY_FINGERPRINTS",
    challengeHistoryChannel: "FORBIDDEN",
    precomputedTerminalHistoryState: "FORBIDDEN",
    heldoutPanelMaterialized: false,
    heldoutSeedGenerated: false,
...
export function diagnoseG5R3TrajectoryLoadBearing() {
  ...
  const withoutEarly = trajectoryDigestFromFingerprints(fingerprints.slice(1));
  const withoutMiddle = trajectoryDigestFromFingerprints([fingerprints[0], fingerprints[2]]);
  const withoutLatest = trajectoryDigestFromFingerprints(fingerprints.slice(0, 2));
  const reversed = trajectoryDigestFromFingerprints([...fingerprints].reverse());
  return Object.freeze({
    diagnosticOnly: true,
    allThreeLoadBearingAtDigestLayer:
      new Set([full, withoutEarly, withoutMiddle, withoutLatest]).size === 4,
    chronologySensitiveAtDigestLayer: full !== reversed,
    heldoutPanelMaterialized: false,
    subjectOutcomeGenerated: false,
  });
}

export function solveG5AutobiographicalHistoryR3({ challenge, memoryProvider } = {}) {
  const normalizedChallenge = normalizeChallenge(challenge);
  if (typeof memoryProvider !== "function") throw new Error("G5_R3_MEMORY_PROVIDER_REQUIRED");
  ...
  const action = actionFromTrajectory({ ...normalizedChallenge, trajectoryDigest });
