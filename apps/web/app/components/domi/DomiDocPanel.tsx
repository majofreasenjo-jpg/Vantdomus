...
export default function DomiDocPanel({
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-lg rounded-3xl border shadow-2xl p-5 max-h-[88vh] overflow-y-auto ${card}`}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold font-display leading-tight">Domi Documental</h3>
              <p className="text-[11px] opacity-60">Domi lee, entiende y propone. Tú confirmas.</p>
            </div>
          </div>
        ...
        {!cand ? (
          <>
            ...
            <div className="flex gap-2 mb-3">
              {([["archivo", "PDF", UploadCloud], ["texto", "Texto", FileText], ["url", "Enlace", Link2]] as const).map(([m, label, Icon]) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border text-xs transition-colors cursor-pointer ${mode === m ? tabActive : tabIdle}`}
                >
            ...
            {mode === "archivo" && (
              <label className={`block rounded-2xl border border-dashed p-5 text-center cursor-pointer ${isLight ? "border-slate-300 hover:bg-slate-50" : "border-slate-700 hover:bg-slate-900/60"}`}>
                ...
                <span className="text-xs opacity-80">{file ? file.name : "Elegí un PDF (receta, boleta, circular, cuenta…)"}</span>
                <input type="file" className="hidden" accept=".pdf,image/*" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                <p className="text-[10px] opacity-50 mt-1.5">PDF con texto = lectura real. Imágenes escaneadas: OCR limitado en esta versión.</p>
              </label>
            )}
            {mode === "texto" && (
              <textarea
                rows={5}
                className={`w-full rounded-2xl border p-3 text-sm outline-none focus:border-amber-500/60 ${inputCls}`}
            ...
            {mode === "url" && (
              <div>
                ...
                />
                <div className="flex items-start gap-1.5 mt-2 text-[11px] text-amber-600/90">
            ...
            {persons.length > 0 && (
              <select
                className={`w-full mt-3 rounded-xl border p-2.5 text-xs outline-none ${inputCls}`}
                value={personId}
            ...
            <button
              ...
              disabled={busy}
              onClick={analyze}
              className="w-full mt-3 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-slate-950 text-sm font-semibold disabled:opacity-60 cursor-pointer hover:brightness-105 transition-all"
            >
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-500/15 text-amber-600 border border-amber-500/30">
                {ROUTE_LABELS[route] || "Documento"}
              </span>
              {cand._demo ? (
                <span className="px-2 py-1 rounded-full text-[10px] font-mono bg-slate-500/15 text-slate-400 border border-slate-500/30">DEMO · sin fetch</span>
              ) : (
                <span className="text-[11px] opacity-60">Confianza {Math.round((cand.confidence || 0) * 100)}%</span>
              )}
            </div>

            <p className="text-sm opacity-90 mb-3">{cand.summary}</p>

            {/* Entidades detectadas */}
            {!cand._demo && (
              <div className={`rounded-2xl border p-3 mb-3 text-xs space-y-1 ${isLight ? "bg-slate-50 border-slate-200" : "bg-slate-900/50 border-slate-800"}`}>
            ...
            {cand._source && (
              <div className="text-[11px] opacity-60 mb-3 break-all">Fuente: {cand._source}</div>
            )}

            {sensitive && !cand._demo && (
              <div className="flex items-start gap-1.5 mb-3 text-[11px] text-amber-600">
                <ShieldAlert className="w-3.5 h-3.5 shrink-0 mt-0.5" />
            ...
            {!cand._demo && NEEDS_PERSON.has(route) && !cand.person_id && persons.length > 0 && (
              <select
                className={`w-full mb-3 rounded-xl border p-2.5 text-xs outline-none ${inputCls}`}
                value={personId}
                onChange={(e) => setPersonId(e.target.value)}
              >
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={busy}
                onClick={confirm}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-sm font-semibold disabled:opacity-60 cursor-pointer hover:brightness-105 transition-all"
              >
              ...
              <button
                ...
                disabled={busy}
                onClick={reject}
                className={`px-4 py-2.5 rounded-xl border text-sm cursor-pointer ${isLight ? "border-slate-300 hover:bg-slate-100" : "border-slate-700 hover:bg-slate-800"}`}
              >
