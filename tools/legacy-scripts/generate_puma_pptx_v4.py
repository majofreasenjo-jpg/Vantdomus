...
out_file = os.path.join(base_dir, "Presentacion_Antucoya_Ejecutiva_v4.pptx")
photos = sorted([f for f in os.listdir(base_dir) if f.startswith("WhatsApp Image")])

def add_timeline(slide):
    # La barra horizontal principal
    bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(1.0), Inches(4.5), Inches(11.0), Inches(0.1))
    bar.fill.solid()
    bar.fill.fore_color.rgb = RGBColor(200, 200, 200)
    bar.line.fill.background()

    # Ahora 4 hitos tras revisión del Gantt
    milestones = [
        {"date": "14-Abr-26", "title": "Día Cero Operativo", "desc": "Ing. Apta + Suministros Cliente\nFin Mov. e Inicio F1 y PLS 1"},
        {"date": "21-Nov-26", "title": "Término Entregas (Mandante)", "desc": "Entrega final F4 Pila Oeste ILS\nDesbloqueo 100% frentes"},
        {"date": "03-Feb-27", "title": "Fin Reparación Piscinas", "desc": "Hito A1030 (PLS/ILS)"},
        {"date": "09-Mar-27", "title": "Fin Entubado Canaletas", "desc": "Hito A1040 (Cierre de Obras)"}
    ]

    # Espaciado para 4 nodos
    spacing = 3.2
    start_x = 1.6
    ...
    for i, m in enumerate(milestones):
        ...
        # Texto Título (Abajo)
        tb_box = slide.shapes.add_textbox(Inches(cx - 1.25), Inches(5.0), Inches(2.5), Inches(1.1))
...
def main():
    ...
    slides_data = [
        ...
        {
            "sub": "⚠️ LÍNEA DE TIEMPO: CONVERGENCIA Y RUTA CRÍTICA",
            "critical": True,
            "has_timeline": True,
            "bullets": [
                "El 14-Abr-26 representa el Hito 'Big Bang' del proyecto: converge Ingeniería Apta, llegadas de Suministros, fin de movilización y liberación por parte del cliente."
            ]
        },
        {
            ...
            "bullets": [
                ...
                "Mientras el Frente 1 estabiliza cañerías, el Frente 2 entra en preparación de sello (> 48 días estimados de limpieza).",
                "Impacto Directo: Reducción del plazo constructivo en un 30% a 40%, optimizando grúas y cuadrillas."
            ]
        },
        {
            ...
            "bullets": [
                ...
                "Retiro masivo de borra (43 días de retiro estimados) y mejoramiento estructural del fondo de fundación.",
        ...
        {
            "sub": "Aseguramiento y Calidad Trazable (Ingeniería y Suministros)",
            "critical": False,
            "has_timeline": False,
            "bullets": [
                "Convergencia de Ingeniería Apta y Control QA/QC mediante vacuómetros y chispa (Spark Tester) constante.",
                "Procura de Suministros Desafiante (HDPE 1200mm y Geotextiles críticos) atada directamente al Día Cero (14-Abr-26).",
